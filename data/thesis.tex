...
Note that the last multiplier is equal to 1 due to expression \ref{eq:autoencoder-derivatives-1}. The first multiplier can be taken as the sign of the integral (it does not contain $z$) in \ref{eq:autoencoder-density}. And also assume that $p(z)$ is a fairly smooth function and does not change much in the neighborhood of $z^*$, i.e. replace $p(z)$ by $p(z^*)$.
...
Suppose we already have an ideal discriminator image. For any image example, it outputs the true probability that this example belongs to a given subset of the image from which the image sample was obtained. Reformulating the task of deceiving the discriminator in probabilistic language, we get that it is necessary to maximize the probability given by the ideal discriminator on the generated examples. Thus, the optimal generator is located as an image. Since image is a monotonically increasing function and does not change the position of the extremums of the argument, this formula should be rewritten in the form of image, which will be convenient in the future.

In reality, there is usually no ideal discriminator and it must be found. Since the task of the discriminator is to provide a signal for training the generator, instead of an ideal discriminator, it is enough to take a discriminator that ideally separates the real examples from those generated by the current generator, i.e. ideal only on a subset of the image from which the examples are generated by the current generator. This task can be reformulated as a search for such an image function that maximizes the probability of correctly classifying examples as real or generated. This is called a binary classification problem, and in this case we have an infinite training sample: a finite number of real examples and a potentially infinite number of generated examples. Each example has a label: it is real or generated. The first article described the solution of the classification problem using the maximum likelihood method. Let's write it down for our case.

So, our sample image. Let's determine the density of the image distribution, then image is a reformulation of the image discriminator, which gives out the probability of the image class (this example) in the form of a distribution on the image classes. Since image, this definition sets the correct probability density. Then the optimal discriminator can be found as:

image

Group the multipliers for image and image:
...
Setting an arbitrary initial image.
The image iteration begins, image.
We are looking for an optimal discriminator for the current generator:

image.
Improving the generator using the optimal discriminator:

image. It is important to be in the vicinity of the current generator. If you move away from the current generator, the discriminator will cease to be optimal and the algorithm will cease to be correct.
The task of training the generator is considered solved when the image for any image. If the process does not converge, then go to the next iteration in point (2).
...
Both image functions can be represented as neural networks: image, after which the task of finding optimal functions is reduced to the task of optimization by parameters and it can be solved using traditional methods: backpropagation and SGD. Additionally, since the neural network is a universal function approximator, image can approximate an arbitrary probability distribution, which removes the question of choosing the image distribution. This can be any continuous distribution within some reasonable limits. For example, image or image. The correctness of this algorithm and the convergence of image to image under fairly general assumptions is proved in the original article.
...
The first model \cite[Learning Deep CNN Denoiser Prior for Image Restoration]{zhang2017learning} proposes a denoiser model that is capable of removing a noise artifacts from images.

The second model \cite[Plug-and-Play Image Restoration with Deep Denoiser Prior]{zhang2020plug} is an improved version of the first one, which is also denoiser.

The third model \cite[Towards Flexible Blind JPEG Artifacts Removal]{jiang_towards_2021} is a general artifact removing model. It is especially useful since it can work in blind manner, which means that no prior knowledge such as quantization table is required to perform a reconstruction and artifact removal.
...
To finetune enhancement models we first pretrain these models on dataset. Then we use our compression model as a source of input data e.g. we place enhancer on top of the model and fine tune the model like this.
